...
function Product({
  ...
  const { getItemToCart } = useContext(AppContext);

  const [isFavorite, setIsFavorite] = useState(favorited);

  function changeIsAdded() {
    onClick();
  }
    ...
    <li>
      {loading ? (
        <ContentLoader
          ...
          // {...props}
        >
          <rect x="0" y="25" rx="10" ry="10" width="150" height="90" />
          <rect x="1" y="127" rx="5" ry="5" width="150" height="15" />
          <rect x="0" y="145" rx="5" ry="5" width="95" height="15" />
          <rect x="0" y="180" rx="5" ry="5" width="80" height="25" />
          <rect x="118" y="173" rx="10" ry="10" width="32" height="32" />
        </ContentLoader>
      ) : (
        <>
          <div className={productStyles.favorite} onClick={changeIsFavorite}>
            {isFavorite ? (
              <svg
                width="32"
                height="32"
          ...
          <img width={133} height={112} src={urlImg} alt="Product" />
          <h3>{title}</h3>
          <div className={productStyles.box}>
            <span>
              Цена:<span className={productStyles.price}>{price} грн.</span>
            </span>
            <button onClick={changeIsAdded}>
              {getItemToCart(id) ? (
                <svg
                  width="32"
                  height="32"
